...
public class Exercise_ComboActionEvent extends JFrame{
	...
	public Exercise_ComboActionEvent() {
		...
		c.add(strCombo);	// 콘텐트팬에 꼭 추가
		c.add(imgLabel);
		strCombo.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				JComboBox<String> strCb = 
						(JComboBox<String>)e.getSource();	// e.getSource()는 이벤트가 발생한 컴포넌트를 가져옴 (this와 비슷)
				int index = strCb.getSelectedIndex();	// 선택된 아이템의 index값을 구해오는 메소드 인데 DOM에도 있음
